...
```
容器拉起后，可在浏览器访问http://127.0.0.1:8080，然后参考**快速使用**部分开始使用。
...
#### 准备工作
- ZooKeeper集群
- tubemq-server-[TUBEMQ-VERSION]-bin.tar.gz安装包
...
#### 启动Master
完成如上配置设置后，进入主备Master所在的TubeMQ环境的 `bin` 目录，启动服务:
```bash
./tubemq master start
```
...
### 运行Example
可以使用 Example 来测试集群。首先，我们运行producer的demo，注意将 `YOUR_MASTER_IP` 替换为实际的IP
```bash
...
YOUR_MASTER_IP:8000 demo 10000
```
从日志我们可以看到，数据发送成功
```bash
...
然后运行 consume 的 demo，`YOUR_MASTER_IP` 也需要替换
```bash
install_path=/INSTALL_PATH/tubemq-server-[TUBEMQ-VERSION]-bin
