...
1.Sort 模块提供实时数据同步能力，负责在各类数据库之间读取与写入受信任的数据。除非另有约定，数据库中的恶意数据均被视为不安全输入，用户应自行确保数据源的安全性，包括但不限于对入库数据进行校验、对访问来源进行鉴权等。

2.Manager 模块提供租户隔离能力。在同一租户内，任意成员均可查看该租户下的全部业务信息，包括 Group、Sink、Stream 等。在租户权限模型下，仅 Group 责任人有权对其名下的 Group、Sink、Stream 等信息进行修改与删除；成员在被授予的权限范围内执行的正常操作亦属此列。如需防止业务信息被他人查看，用户只需确保未将无关人员加入当前租户。
